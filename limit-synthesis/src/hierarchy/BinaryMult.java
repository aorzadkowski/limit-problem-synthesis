...
public class BinaryMult implements BinaryOperator
{
    ...
    public boolean validate()
    {
    	...
    	if(LimitExpression.PROHIBIT_NUM_BO_NUM)
    	{
    		...
    		{
    			if(_exp1.unParse().equalsIgnoreCase("#Pi") || _exp2.unParse().equalsIgnoreCase("#Pi"))
    			{
    	...
    	if(LimitExpression.PROHIBIT_PI_TIMES_NUM)
    	{
    		if(_exp1 instanceof Number && _exp2 instanceof Number)
    		{
    			if(_exp1.unParse().equalsIgnoreCase("#Pi"))
    			{
    				validate = false;
        			return validate;
    			}
    	...
    	if(LimitExpression.PROHIBIT_NUM_TIMES_PI)
    	{
    		if(_exp1 instanceof Number && _exp2 instanceof Number)
    		{
    			if(_exp2.unParse().equalsIgnoreCase("#Pi"))
    			{
    				validate = false;
        			return validate;
    			}
    	...
    	if(LimitExpression.PROHIBIT_NUM_BO_EXP)
    	{
    	...
    	if(LimitExpression.PROHIBIT_X_BO_EXP)
    	{
    	...
    	if(LimitExpression.PROHIBIT_EXP_BO_X)
    	{
    	...
    	if(LimitExpression.PROHIBIT_EXP_BO_EXP)
    	{
    ...
    public Expression deepCopy()
    {
    	...
    	return new BinaryMult(newExp1, newExp2);
    }
    
    public boolean equals(Expression other) {
    	if (other == null) return false;
    	if (!(other instanceof BinaryMult)) return false;
    	if (((BinaryMult)other).getExp1().equals(_exp1) && ((BinaryMult)other).getExp2().equals(_exp2)) {
    		return true;
    	}
    	return false;
    }
